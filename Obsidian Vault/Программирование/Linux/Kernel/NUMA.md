...
*   **Действие ядра:** Ядро находит свободный диапазон виртуальных адресов в вашем процессе и записывает в его таблицы страниц запись, которая говорит: "с этого адреса начинается участок памяти, но физической страницы за ним пока нет".
*   **Счетчик памяти:** Ядро увеличивает счетчик выделенной виртуальной памяти процесса (`vm_committed_space`), но **не выделяет** физическую оперативную память (RAM).
*   **Результат:** Вы получаете указатель на виртуальный адрес. Физическая память (и выбор NUMA-узла) пока не задействована.
...
### 2. Момент истины — Page Fault

Физическая память появляется только в момент первого обращения к байту по этому адресу (например, `allocated_ptr[0] = 'X';`):
1.  **CPU (MMU):** Пытается прочитать инструкцию по виртуальному адресу, проверяет таблицы страниц и видит, что физическая страница отсутствует.
2.  **Прерывание:** CPU генерирует **Page Fault** и передает управление ядру Linux.
3.  **Обработчик ядра:** Срабатывает механизм **Demand Paging** (подкачка по требованию). Ядро выделяет физическую страницу RAM, очищает её (если нужно) и обновляет таблицы страниц процесса, связывая виртуальный адрес с реальным физическим адресом.

**Именно в этот момент (шаг 3) и происходит магия NUMA.**

### 3. NUMA и Правило «Первого касания» (First Touch)

При стандартных настройках Linux ядро смотрит на **какой именно CPU (core) выполнил инструкцию, вызвавшую Page Fault**.
